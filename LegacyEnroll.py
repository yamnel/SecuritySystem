...
def LegacyEnroll(fps):
    ...
    if enrollid < 200:
        ...
        print
        ('Press finger to Enroll %s' % str(enrollid))
        ...
        if fps.CaptureFinger(True):
            ...
            if fps.CaptureFinger(True):
                ...
                if fps.CaptureFinger(True):
                    ...
                    if iret == 0:
                        print
                        ('Enrolling Successfull')
                    else:
                        print
                        ('Enrolling Failed with error code: %s' % str(iret))
                else:
                    print
                    ('Failed to capture third finger')
            else:
                print
